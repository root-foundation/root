...
From this _root_, all activity blossoms.
...
We believe this can _only_ be realized through a _single_ system.
...
Thus, AGI will _converge_ all activities towards a _singular_ activity.

_Footnote: This is why the claim that AGI will create more jobs than it takes is naive: it ignores the fact that all of those new jobs will be more complex with far more extreme outcomes, and far more similar to each other such that it is that much tougher to be one of the best because the pool of competition is exponentially greater._
...
[^4]: We should not be attached to these labels: "designer", "salesperson", etc. How complex these roles are is dependent on the nature of the company. For example, a lead designer at a much more ambitious startup will operate at a much higher level of complexity than a designer at a more narrowly focused company.
...
FoundationOrg members will collaborate _in person_ at our HQ in San Francisco, California, USA.

### Competition

To encourage competition, FoundationOrg will enable and encourage many for-profit entities to operate on _top_ of it to create a rich ecosystem of products and services that engage with the core layer.
...
In the early stages of any category, FoundationOrg and FirstCompany will control a lot of the power. Over time, as the category matures, FoundationOrg will progressively decentralize control. For example, in the context of personal tokens, early on the FoundationOrg / FirstCompany will handle reputation and fraud in a centralized way: managing complaints from users, investigating and banning / pursuing legal actions, etc. Over time, as the personal token ecosystem matures, FoundationOrg will decentralize reputation by incentivizing users themselves to accurately identify misrepresentation / fraud[^6].
...
We will start with personal tokens, and the new economic system upon it. Then, we will implement governance instruments that have UBI + UBE at their core to create a symbiotic relationship between the individual and the collective to enable us to grow _together_. There are many other stories and instruments we will need to implement. We will share these plans when the time is right.

Finally, upon this Foundation, we will converge towards _superintelligence_.
